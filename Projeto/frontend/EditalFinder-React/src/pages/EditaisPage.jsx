...
import {
  ...
  logEditaisQueryFiltersApplied,
  normalizeScopeParam,
  parseEditaisQueryFilters,
  prazoFilterLabel,
  presetToPrazoQuery,
  SCOPE_FILTER_TODOS,
  scopeFilterLabel,
  sidebarToSearchParams,
} from '../utils/editais/editaisQueryFilters';
import { editalMatchesPrazoQuery } from '../utils/editais/editalDeadlineStatus';
...
export default function EditaisPage() {
  ...
  useEffect(() => {
    const parsed = parseEditaisQueryFilters(searchParams);
    logEditaisQueryFiltersApplied(parsed);
    skipUrlSyncRef.current = true;
    setFilters((f) => applyQueryFiltersToSidebar(f, parsed));
  }, [location.search, searchParams]);

  const queryFilterSignature = useMemo(
    () =>
      JSON.stringify({
        prazoPreset: filters.prazoPreset,
        queryScope: filters.queryScope,
        queryModalidade: filters.queryModalidade,
        queryFonte: filters.queryFonte,
        fonteBusca: filters.fonteBusca,
      }),
    [
      filters.prazoPreset,
      filters.queryScope,
      filters.queryModalidade,
      filters.queryFonte,
      filters.fonteBusca,
    ],
  );
  ...
  useEffect(() => {
    ...
    const nextParams = sidebarToSearchParams(filters);
    const cur = searchParams.toString();
    const next = nextParams.toString();
    if (next !== cur) {
      setSearchParams(nextParams, { replace: true });
    }
  }, [queryFilterSignature, searchParams, setSearchParams]);

  const scopeFilter = useMemo(
    () => normalizeScopeParam(filters.queryScope || searchParams.get('scope')),
    [filters.queryScope, location.search],
  );
  ...
  useEffect(() => {
    if (!import.meta.env.DEV || !filters.prazoPreset) return;
    const prazo = presetToPrazoQuery(filters.prazoPreset) || filters.prazoPreset;
    const before = allEditais.length;
    const after = allEditais.filter((e) => editalMatchesPrazoQuery(e, prazo)).length;
    logEditaisDeadlineFilterResult({
      prazo,
      before,
      after,
      listed: filteredEditaisRaw.length,
    });
  }, [filters.prazoPreset, allEditais.length, filteredEditaisRaw.length]);
  ...
  const chips = useMemo(() => {
    ...
    if (filters.prazoPreset) {
      const prazoQ = presetToPrazoQuery(filters.prazoPreset) || filters.prazoPreset;
      out.push({
        key: 'prazo-q',
        label: prazoFilterLabel(prazoQ),
        onRemove: rm(() =>
          setFilters((f) => ({
            ...f,
            prazoPreset: '',
    ...
    if (scopeFilter !== SCOPE_FILTER_TODOS) {
      out.push({
        key: 'scope-q',
        label: scopeFilterLabel(scopeFilter),
        onRemove: rm(() =>
          setFilters((f) => ({
            ...f,
            queryScope: '',
    ...
    if (filters.queryModalidade) {
      out.push({
        key: 'mod-q',
        label: `Modalidade: ${filters.queryModalidade}`,
        onRemove: rm(() =>
          setFilters((f) => ({
            ...f,
            queryModalidade: '',
