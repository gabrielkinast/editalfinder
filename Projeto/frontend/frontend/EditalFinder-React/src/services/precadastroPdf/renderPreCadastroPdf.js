...
const M = 14;
const BOTTOM = 26;
/** Conteúdo deve terminar acima da faixa reservada ao rodapé grafado em `stampFooters`. */
function contentMaxY(doc) {
  return doc.internal.pageSize.getHeight() - BOTTOM - 14;
}

/**
 * Garante espaço vertical; quebra página se necessário.
 * @returns {number} Y seguro para começar a desenhar
 */
function ensureVerticalSpace(doc, y, neededMm) {
  const maxY = contentMaxY(doc);
  if (y + neededMm <= maxY) return y;
  doc.addPage();
  return M + 14;
}

/**
 * Desenha texto multilinha com quebra de página automática.
 * @returns {number} Y após o último fragmento
 */
function renderMultilineParagraph(doc, lines, x, yStart, lineHeight = 4.35) {
  let y = yStart;
  const maxY = contentMaxY(doc);
...
export function stampFooters(doc, clientNameShort) {
  ...
  for (let i = 1; i <= n; i += 1) {
    ...
    doc.text(leftLine.slice(0, 118), M, ph - BOTTOM + 15);
    doc.text(`Pagina ${i} de ${n}`, pw - M, ph - BOTTOM + 15, { align: 'right' });
...
export function renderPreCadastroDocument(doc, model, theme, rawForm) {
  ...
  doc.addPage();
  const yImpTop = banner(doc, theme, M + 12, 'Impactos esperados (declarativos)');
  let yImpactNext = yImpTop + 4;
  const impactBlocks = [['Impactos economicos:', IMP_ECO], ['Impactos sociais:', IMP_SOC], ['Impactos ambientais:', IMP_AMB]];
  for (let ii = 0; ii < impactBlocks.length; ii += 1) {
    const [title, list] = impactBlocks[ii];
    yImpactNext = ensureVerticalSpace(doc, yImpactNext, 32);
    autoTable(doc, {
      startY: yImpactNext,
      margin: { left: M, right: M, bottom: BOTTOM + 8 },
      head: [[`${title}`, '']],
      headStyles: {
      ...
      columnStyles: { 0: { cellWidth: 18 }, 1: { cellWidth: 'auto' } },
      styles: { fontSize: 7.9, overflow: 'linebreak' },
      theme: 'plain',
      lineWidth: 0,
    });
    yImpactNext = doc.lastAutoTable.finalY + 10;
  }
  ...
  let yLic = banner(doc, theme, M + 12, 'Licencas condicoes financiamento e declaracao');
  yLic = banner(doc, theme, yLic + 4, 'Licencas e autorizacoes');
  autoTable(doc, {
    startY: yLic + 4,
    margin: { left: M, right: M, bottom: BOTTOM + 8 },
    styles: {
  ...
  yPos = banner(doc, theme, yPos, 'Condicoes complementares');
  ...
  let yDecl = doc.lastAutoTable.finalY + 12;
  yDecl = ensureVerticalSpace(doc, yDecl, 90);
  yDecl = banner(doc, theme, yDecl, 'Declaracao e assinatura');

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  const declLines = doc.splitTextToSize(`${model.declaracao.texto}`, doc.internal.pageSize.getWidth() - 2 * M - 4);
  let yAfterText = renderMultilineParagraph(doc, declLines, M + 2, yDecl + 4, 4.35);
  yAfterText += 10;

  const sigBoxH = 52;
  yAfterText = ensureVerticalSpace(doc, yAfterText, sigBoxH + 14);
  const pw = doc.internal.pageSize.getWidth();
  doc.setDrawColor(...theme.borderColor);
  doc.setLineWidth(0.35);
  doc.rect(M + 10, yAfterText, pw - 2 * M - 20, sigBoxH);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.setTextColor(...[15, 23, 42]);
  doc.text('Espaco para assinatura / carimbo', M + 14, yAfterText + 8);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8.5);
  doc.text(`${model.declaracao.localData}`, M + 14, yAfterText + 20);
  doc.text(`Nome: ${model.declaracao.responsavel}`, M + 14, yAfterText + 30);
  doc.text(`Cargo: ${model.declaracao.cargo}`, M + 14, yAfterText + 40);
  doc.line(M + 14, yAfterText + 46, pw - M - 14, yAfterText + 46);
  doc.setFontSize(7.5);
  doc.setTextColor(...[100, 116, 139]);
  doc.text('Assinatura do responsavel legal', M + 14, yAfterText + 50);
  doc.setTextColor(...[15, 23, 42]);
}
