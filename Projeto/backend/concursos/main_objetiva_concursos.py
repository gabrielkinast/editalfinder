...
# -*- coding: utf-8 -*-
"""
Crawler piloto — Objetiva Concursos (portal oficial Instituto Objetiva).

Diagnóstico (2026):
- Portal: `https://concursos.objetivas.com.br` — CMS selecao.net (mesma família Quadrix/IBFC).
- Listagens: `/index/abertos/`, `/index/1/` (em andamento). Menu também encerrados (`/index/3/`).
...
Saída standardized para `scripts/load_concursos_selecao.py` (`fonte=objetiva`).
"""
...
BASE = "https://concursos.objetivas.com.br"
DEFAULT_LISTINGS = (
...
def objetiva_should_discard_situacao(situacao_lower: str) -> Tuple[bool, str]:
    if "encerr" in situacao_lower or "finaliz" in situacao_lower:
        return True, "objetiva_situacao_encerrada"
    if "cancel" in situacao_lower:
        return True, "objetiva_situacao_cancelada"
    if "suspen" in situacao_lower:
        return True, "objetiva_situacao_suspensa"
    return False, ""


def objetiva_should_discard_non_opportunity(*, titulo: str, body: str) -> Tuple[bool, str]:
    blob = f"{titulo}\n{body}".lower()
    for pat in _RE_NON_OPP:
        if re.search(pat, blob, re.I):
            return True, "objetiva_nao_oportunidade_ativa"
...
def parse_objetiva_page(url: str) -> Dict[str, Any]:
    html = _fetch(url)
    soup = BeautifulSoup(html, "html.parser")
...
def run_crawl(
    *,
    ...
    for url in urls:
        ...
        try:
            time.sleep(sleep_s)
            parsed = parse_objetiva_page(url)
            tit = parsed["titulo"]
            body = parsed["body"]

            drop_s, why_s = objetiva_should_discard_situacao(parsed.get("situacao_lower") or "")
            if drop_s:
                discarded.append({"url": url, "titulo": tit, "motivo": why_s})
                continue

            drop_no, why_no = objetiva_should_discard_non_opportunity(titulo=tit, body=body)
            if drop_no:
            ...
            extras: Dict[str, Any] = {
                "crawler": "main_objetiva_concursos",
                "wave": "concursos_wave1_objetiva",
                "fetched_at_utc": datetime.now(timezone.utc).isoformat(),
                "objetiva_listing_urls": list(listing_urls),
                ...
                "objetiva_situacao": (parsed.get("situacao_lower") or "")[:200],
            }
            row = build_concurso_item(
                ...
                extras=extras,
                categoria="banca_objetiva_concurso",
                orgao=orgao,
                instituicao=instituicao,
                banca=BANCA,
                cargo=cargo_txt,
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Crawler piloto Objetiva Concursos → standardized JSON")
    ...
    ap.add_argument(
        "--output-root",
        type=str,
        default=str(ROOT / "audit_reports_main_pipeline/concursos_wave1_objetiva"),
        help="Pasta wave1 Objetiva",
    )
    args = ap.parse_args()
