...
Diagnóstico (2026):
- Portal oficial: `https://concursos.ibfc.org.br` — mesma família de CMS que Quadrix (selecao.net).
- Listagens: `/index/abertos/`, `/index/1/` (em andamento). Rotas `/index/3/` etc. (encerrados)
  existem no menu; piloto limita-se a abertos + em andamento.
- Detalhe: `/informacoes/{id}/` — HTML ISO-8859-1 típico; `#TopoInformacoes`, `p.insc`, publicações PDF,
  `#blocoEventos`, `#blocoListaVagas` (mesmos seletores que Quadrix).
- robots.txt: Allow `/`; Disallow `/admin/*`, `/painel/*`, `/uploads/*`.

Saída standardized para `scripts/load_concursos_selecao.py` (`fonte=ibfc`).
"""
...
def ibfc_should_discard_situacao(situacao_lower: str) -> Tuple[bool, str]:
    if "encerr" in situacao_lower or "finaliz" in situacao_lower:
        return True, "ibfc_situacao_encerrada"
    if "cancel" in situacao_lower:
        return True, "ibfc_situacao_cancelada"
    if "suspen" in situacao_lower:
        return True, "ibfc_situacao_suspensa"
    return False, ""


def ibfc_should_discard_non_opportunity(*, titulo: str, body: str) -> Tuple[bool, str]:
    blob = f"{titulo}\n{body}".lower()
    for pat in _RE_NON_OPP:
        if re.search(pat, blob, re.I):
            return True, "ibfc_nao_oportunidade_ativa"
...
def parse_ibfc_page(url: str) -> Dict[str, Any]:
    html = _fetch(url)
    soup = BeautifulSoup(html, "html.parser")
...
def run_crawl(
    *,
    ...
    for url in urls:
        ...
        try:
            time.sleep(sleep_s)
            parsed = parse_ibfc_page(url)
            tit = parsed["titulo"]
            body = parsed["body"]

            drop_s, why_s = ibfc_should_discard_situacao(parsed.get("situacao_lower") or "")
            if drop_s:
                discarded.append({"url": url, "titulo": tit, "motivo": why_s})
                continue

            drop_no, why_no = ibfc_should_discard_non_opportunity(titulo=tit, body=body)
            if drop_no:
            ...
            extras: Dict[str, Any] = {
                "crawler": "main_ibfc_concursos",
                "wave": "concursos_wave1_ibfc",
                "fetched_at_utc": datetime.now(timezone.utc).isoformat(),
                "ibfc_listing_urls": list(listing_urls),
                ...
                "ibfc_situacao": (parsed.get("situacao_lower") or "")[:200],
            }
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Crawler piloto IBFC → standardized JSON")
    ...
    ap.add_argument(
        "--output-root",
        type=str,
        default=str(ROOT / "audit_reports_main_pipeline/concursos_wave1_ibfc"),
        help="Pasta wave1 IBFC",
    )
    args = ap.parse_args()
