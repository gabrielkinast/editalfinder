...
# -*- coding: utf-8 -*-
"""
Crawler piloto — Instituto Quadrix (HTML oficial quadrix.org.br / plataforma selecao.net.br).

Diagnóstico (2026):
- Listagens: `https://www.quadrix.org.br/index/abertos/` (inscrições abertas),
  `https://www.quadrix.org.br/index/1/` (em andamento). Menu também referencia
  `/index/3/` (encerrados), `/index/4/` (suspenso), `/index/6/` (em breve) — não usadas no piloto.
- Detalhe: `https://www.quadrix.org.br/informacoes/{id}/` — HTML com charset frequentemente ISO-8859-1.
- Campos úteis: `#TopoInformacoes .dados h2` (órgão/cliente), `p.tipo`, `p.periodoInscricoes`,
  `p.insc` (inscrições com horário), `p.situacaoConcurso`, bloco Publicações (`li.pdf` + PDF CDN),
  cronograma `#blocoEventos table.tabela`, vagas `#blocoListaVagas`.
- robots.txt: Allow `/`; Disallow `/admin/*`, `/painel/*`, `/uploads/*` — não seguir `/painel/`.

Saída standardized para `scripts/load_concursos_selecao.py` (`fonte=quadrix`).
"""
...
BASE = "https://concursos.quadrix.org.br"
BASE_LEGACY = "https://www.quadrix.org.br"
MAX_LISTING_PAGES_DEFAULT = 3
DEFAULT_LISTINGS = (
    f"{BASE}/index/abertos/",
    f"{BASE}/index/1/?ord=&dir=&pg=2&q=&pp=10",
    f"{BASE}/index/1/?ord=&dir=&pg=3&q=&pp=10",
)
_QUADRIX_NETLOCS = frozenset(
    {
        "concursos.quadrix.org.br",
        "www.quadrix.org.br",
        "quadrix.org.br",
    }
)
...
def _collect_informacao_urls(html: str, listing_url: str, robots_parser) -> List[str]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if netloc not in _QUADRIX_NETLOCS:
            continue
...
def _pick_edital_pdf(soup: BeautifulSoup) -> Optional[str]:
    ...
    for li in root.select("li.pdf"):
        ...
        if re.search(r"(?i)\bedital\b", label) and "cronograma" not in label:
            return href.split("#")[0]
        if "edital" in label and best is None:
            best = href.split("#")[0]
    return best
...
def quadrix_should_discard_situacao(situacao_lower: str) -> Tuple[bool, str]:
    if "encerr" in situacao_lower or "finaliz" in situacao_lower:
        return True, "quadrix_situacao_encerrada"
    if "cancel" in situacao_lower:
        return True, "quadrix_situacao_cancelada"
    if "suspen" in situacao_lower:
        return True, "quadrix_situacao_suspensa"
    return False, ""


def quadrix_should_discard_non_opportunity(*, titulo: str, body: str) -> Tuple[bool, str]:
    blob = f"{titulo}\n{body}".lower()
    for pat in _RE_NON_OPP:
        if re.search(pat, blob, re.I):
            return True, "quadrix_nao_oportunidade_ativa"
...
def parse_quadrix_page(url: str) -> Dict[str, Any]:
    html = _fetch(url)
    soup = BeautifulSoup(html, "html.parser")
...
def _dedupe_rows_by_link(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    seen: Set[str] = set()
    out: List[Dict[str, Any]] = []
    for row in rows:
        lk = (row.get("link") or "").split("#")[0].rstrip("/")
...
def run_crawl(
    *,
    ...
    for robots_url in (f"{BASE}/robots.txt", f"{BASE_LEGACY}/robots.txt"):
        try:
            r2 = RobotFileParser()
    ...
    for url in urls:
        ...
        try:
            time.sleep(sleep_s)
            parsed = parse_quadrix_page(url)
            tit = parsed["titulo"]
            body = parsed["body"]

            drop_s, why_s = quadrix_should_discard_situacao(parsed.get("situacao_lower") or "")
            if drop_s:
                discarded.append({"url": url, "titulo": tit, "motivo": why_s})
                continue

            drop_no, why_no = quadrix_should_discard_non_opportunity(titulo=tit, body=body)
            if drop_no:
            ...
            extras: Dict[str, Any] = {
                "crawler": "main_quadrix_concursos",
                "wave": "concursos_wave1_quadrix",
                "fetched_at_utc": datetime.now(timezone.utc).isoformat(),
                "quadrix_listing_urls": list(listing_urls),
                ...
                "quadrix_situacao": (parsed.get("situacao_lower") or "")[:200],
            }
            row = build_concurso_item(
                ...
                extras=extras,
                categoria="banca_quadrix_concurso",
                orgao=orgao,
                instituicao=instituicao,
                banca=BANCA,
                cargo=cargo_txt,
    ...
    raw_rows = _dedupe_rows_by_link(raw_rows)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Crawler piloto Quadrix → standardized JSON")
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="URLs de listagem separadas por vírgula (vazio = abertos + pg 2..N)",
    )
    ap.add_argument(
        "--max-listing-pages",
        type=int,
        default=MAX_LISTING_PAGES_DEFAULT,
        help="Páginas extras em /index/1/?pg=N (máx. conservador)",
    )
    ap.add_argument(
        "--output-root",
        type=str,
        default=str(ROOT / "audit_reports_main_pipeline/concursos_wave1_quadrix"),
        help="Pasta wave1 Quadrix",
    )
    args = ap.parse_args()

    if str(args.listings).strip():
        listing_urls = tuple(x.strip() for x in str(args.listings).split(",") if x.strip())
    else:
        pages = max(1, min(int(args.max_listing_pages), 5))
        listing_urls = (f"{BASE}/index/abertos/",)
        for pg in range(2, pages + 1):
            listing_urls += (f"{BASE}/index/1/?ord=&dir=&pg={pg}&q=&pp=10",)
    ...
    md = [
        "# Crawler piloto — Quadrix (Instituto Quadrix)",
        "",
        f"- **Fonte:** `{FONT}`",
