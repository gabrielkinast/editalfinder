...
class FapergsScraper:
    ...
    def _article_candidates(self, soup: Union[BeautifulSoup, Tag]) -> list[Tag]:
        found: list[Tag] = []
        seen = set()
    ...
    def _best_title_from_article(self, article: Tag) -> str:
    ...
    def _best_url_from_article(self, article: Tag, base_url: str) -> Optional[str]:
        links = []
    ...
    def extract_open_entries(self) -> list[tuple[str, str]]:
        ...
        if section and section.has_attr("data-matriz-source-uri"):
            service_uri_raw = section["data-matriz-source-uri"]
            service_uri = (
                " ".join(service_uri_raw)
                if isinstance(service_uri_raw, list)
                else str(service_uri_raw)
            ).strip()
    ...
    def parse_entry_page(self, title_hint: str, page_url: str) -> Optional[EditalFapergs]:
        ...
        page_title = first_nonempty([
            clean_text(h1_tag.get_text(" ", strip=True)) if h1_tag and isinstance(h1_tag, Tag) else "",
            clean_text(title_tag.get_text(" ", strip=True)) if title_tag and isinstance(title_tag, Tag) else "",
            title_hint,
        ]) or "Sem título"
