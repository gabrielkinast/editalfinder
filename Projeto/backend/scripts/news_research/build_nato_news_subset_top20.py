#!/usr/bin/env python3
"""
Subset top-20 NATO News para apply staging (sem apply).

Origem: audit_reports_news_research/nato_news_dryrun_100/standardized/
Comparação 30 vs 100 em nato_news_dryrun_100/comparacao_dryrun_30_vs_100.*
"""
...
SOURCE_ID = "nato_news"
DRYRUN_30 = ROOT / "audit_reports_news_research" / "nato_news_dryrun"
DRYRUN_100 = ROOT / "audit_reports_news_research" / "nato_news_dryrun_100"
ORIGIN = DRYRUN_100 / "standardized" / "nato_news_standardized.json"
SUMMARY_30 = DRYRUN_30 / "summary.json"
SUMMARY_100 = DRYRUN_100 / "summary.json"
OUT_BASE = ROOT / "audit_reports_news_research" / "nato_news_subset_top20"
STD_OUT = OUT_BASE / "standardized" / "nato_news_standardized.json"
INPUT_DIR_REL = "audit_reports_news_research/nato_news_subset_top20"
TOP_N = 20
WINDOW_MONTHS = 12
DISPLAY_FONTE = "NATO"
...
def _examples_from_review(path: Path, n: int = 5) -> List[Dict[str, Any]]:
    if not path.is_file():
        return []
        ...
        out.append(
            {
                ...
                "relevancia_nato": it.get("relevancia_nato"),
                "motivos_relevancia": (it.get("motivos_relevancia") or [])[:6],
...
def _build_comparison(s30: Dict[str, Any], s100: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "dryrun_30_dir": "audit_reports_news_research/nato_news_dryrun",
        "dryrun_100_dir": "audit_reports_news_research/nato_news_dryrun_100",
        "metricas": {
            "max_items": {"dryrun_30": 30, "dryrun_100": 100},
            "total_raw": {
                "dryrun_30": t30.get("total_raw"),
                "dryrun_100": t100.get("total_raw"),
                "delta": _delta(t30.get("total_raw"), t100.get("total_raw")),
            },
            "standardized": {
            ...
            "review": {
                "dryrun_30": t30.get("review"),
                "dryrun_100": t100.get("review"),
                "delta": _delta(t30.get("review"), t100.get("review")),
            },
            "errors_count": {
                "dryrun_30": t30.get("errors_count"),
                "dryrun_100": t100.get("errors_count"),
            },
        },
        "eixos_estrategicos": {
        ...
        "relevancia_nato": {
            "dryrun_30": d30.get("relevancia_nato") or {},
            "dryrun_100": d100.get("relevancia_nato") or {},
        },
        "exemplos": {
            "mantidos_30": _examples_from_summary(s30, "mantidos"),
            "mantidos_100": _examples_from_summary(s100, "mantidos"),
            "descartados_30": _examples_from_summary(s30, "descartados")
            or _examples_from_discard(DRYRUN_30 / "descartados_ruido.json"),
            "descartados_100": _examples_from_summary(s100, "descartados")
            or _examples_from_discard(DRYRUN_100 / "descartados_ruido.json"),
            "review_30": _examples_from_summary(s30, "review")
            or _examples_from_review(DRYRUN_30 / "review_candidates.json"),
            "review_100": _examples_from_summary(s100, "review")
            or _examples_from_review(DRYRUN_100 / "review_candidates.json"),
        },
        "avaliacao": {
        ...
        "observacao_fonte": (
            "Fonte oficial internacional da OTAN (NATO); "
            "não é jornalismo independente. Aplicar apenas lote controlado (subset)."
        ),
    }
...
def _comparison_md(comp: Dict[str, Any]) -> str:
    m = comp["metricas"]
    lines = [
        "# NATO News — comparação dry-run 30 vs 100",
        "",
    ...
    for key in ("total_raw", "standardized", "total_valido", "descartados_ruido", "review"):
        row = m[key]
        delta = row.get("delta")
    ...
    lines.extend(["", "## Exemplos review (100)", ""])
    for ex in comp["exemplos"].get("review_100") or []:
        lines.append(
            f"- {ex.get('titulo', '')[:80]} — rel={ex.get('relevancia_nato')} — {ex.get('motivo', '')}"
        )
...
def _build_payload_noticia(validos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in validos:
        ...
        ex["subset_policy"] = "lote_controlado_fonte_oficial_internacional"
        row["extras"] = ex
...
def main() -> int:
    if not ORIGIN.is_file():
        ...
        print(
            "Execute primeiro: python scripts/news_research/run_nato_news_dryrun.py "
            "--max-items 100 --output-dir audit_reports_news_research/nato_news_dryrun_100",
            file=sys.stderr,
        )
        return 1
    ...
    (OUT_BASE / "nato_news_payload_noticia.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (OUT_BASE / "nato_news_payload_pesquisa.json").write_text("[]", encoding="utf-8")
    (OUT_BASE / "nato_news_review_candidates.json").write_text("[]", encoding="utf-8")
    ...
    consolidado: Dict[str, Any] = {
        ...
        "dryrun_ampliado": {
            ...
            "review": t100.get("review"),
            "descartados_ruido": t100.get("descartados_ruido"),
            "errors_count": t100.get("errors_count"),
        },
        "comparacao_30_vs_100": str(
            (DRYRUN_100 / "comparacao_dryrun_30_vs_100.json").relative_to(ROOT)
        ).replace("\\", "/"),
        "subset": {
            "criterios": [
                ...
                f"top {TOP_N}",
                "apenas standardized (exclui review_candidates)",
            ],
        ...
        "itens": [
            {
                ...
                "link": it.get("link"),
                "relevancia_nato": (it.get("extras") or {}).get("relevancia_nato"),
                "eixo_estrategico": it.get("eixo_estrategico"),
            }
            for it in subset
        ],
        "observacao": (
            "Fonte oficial/institucional da OTAN (NATO). "
    ...
    md = [
        "# NATO News — subset top 20 (staging)",
        "",
        ...
        f"| Review | {consolidado['dryrun_ampliado']['review']} |",
    ...
    for i, row in enumerate(consolidado["itens"], 1):
        md.append(
            f"{i}. **{row.get('data_publicacao')}** ({row.get('relevancia_nato')}) — "
            f"{row.get('titulo', '')[:90]}"
        )
