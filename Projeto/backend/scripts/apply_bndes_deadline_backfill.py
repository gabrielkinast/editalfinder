...
from bndes_controlled_apply import apply_bndes_candidates, load_candidates_file  # noqa: E402
from deadline_backfill import controlled_apply_enabled, deadline_backfill_enabled  # noqa: E402


def main() -> int:
    ap = argparse.ArgumentParser(description="Apply controlado BNDES deadlines (10.2D)")
    ap.add_argument(
        "--input",
        type=Path,
        default=ROOT / "outputs" / "bndes_deadline_apply" / "candidates_to_update.json",
    )
    ...
    print(
        f"[apply_bndes_deadline_backfill] ok={result['ok']} "
