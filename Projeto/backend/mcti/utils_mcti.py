...
def get_soup(url: str) -> Optional[BeautifulSoup]:
    try:
        # Tenta com curl como fallback para contornar bloqueios de bots
        cmd = [
            'curl', '-L', '-k',
        ...
        if result.returncode == 0 and result.stdout:
            # Verifica se o conteúdo parece HTML válido e não apenas uma página de erro/cookie
            if "<html" in result.stdout.lower():
                return BeautifulSoup(result.stdout, "html.parser")
            
        # Fallback para requests se curl falhar
