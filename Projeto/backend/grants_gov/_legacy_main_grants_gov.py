"""Implementação legada: api.grants.gov search2 + view-opportunity.html (não usar em produção)."""
...
def fetch_grants() -> List[Dict[str, object]]:
    ...
    for page in range(MAX_PAGES):
        ...
        for row in rows:
            ...
            extras["metodo_extracao"] = "api_rest_legacy"
            extras["legacy_source_variant"] = "grants_legacy_view_opportunity"
            items.append(
                {
                    ...
                    "link": link,
                    "fonte": "Grants.gov",
                    "data_publicacao": str(row.get("postDate") or row.get("postedDate") or "")[:10]
                    or datetime.now().date().isoformat(),
                    "fim_inscricao": str(row.get("closeDate") or row.get("closeDateFormatted") or "")[:10]
                    or None,
                    "situacao": "Aberto",
...
def main() -> None:
    print("[GRANTS_GOV_LEGACY] Iniciando coleta por API legada...")
    items = fetch_grants()
    save_outputs(Path(__file__).parent, "grants_gov", items)
    print(f"[GRANTS_GOV_LEGACY] Registros salvos: {len(items)}")
