"""
Apply controlado de deadlines BNDES (Backend 10.2D).
...
from bndes_detail_recrawl import stats_for_bndes_recrawl_items
from bndes_submission_deadline import SUBMISSION_DEADLINE_KIND
...
BNDES_SOURCE_KEY = "bndes"
BNDES_FONTES = frozenset({"bndes", "banco nacional de desenvolvimento"})
CONTROLLED_APPLY_VERSION = "bndes_controlled_apply_10.2d"
DEFAULT_MAX_APPLY_BATCH = 50
DRY_RUN_REPORT_MAX_AGE_HOURS = 72

OUT_DIR_NAME = "bndes_deadline_apply"
...
PDF_CANDIDATES_FILENAME = "pdf_candidates.json"


def is_bndes_record(record: Dict[str, Any]) -> bool:
    fonte = str(record.get("fonte") or record.get("fonte_recurso") or "").strip().lower()
    if "bndes" in fonte:
        return True
    return normalize_backfill_source(record) == BNDES_SOURCE_KEY
...
def _has_bndes_deadline_in_payload(item: Dict[str, Any]) -> bool:
    ex = _get_extras(item)
    if ex.get("deadline_kind") and ex.get("deadline_kind") != SUBMISSION_DEADLINE_KIND:
        return False
    if ex.get("deadline_kind") == SUBMISSION_DEADLINE_KIND and (
        ex.get("bndes_deadline_iso") or item.get("fim_inscricao") or item.get("prazo_envio")
    ):
        return True
    preview = ex.get("_deadline_backfill_preview") or {}
    status = preview.get("deadline_backfill_status")
    if status in ("new_deadline_found", "corrected_deadline_found") and preview.get("prazo_envio"):
        return ex.get("deadline_kind") == SUBMISSION_DEADLINE_KIND
    backfill = extract_source_deadline_fields(item)
    if backfill.get("deadline_backfill_status") not in (
        "new_deadline_found",
        "corrected_deadline_found",
    ):
        return False
    patch = backfill.get("extras_patch") or {}
    return patch.get("deadline_kind") == SUBMISSION_DEADLINE_KIND
...
def snapshot_deadline_state(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "extras_bndes_deadline_iso": extras.get("bndes_deadline_iso"),
        "extras_deadline_source_field": extras.get("deadline_source_field"),
        "sem_prazo_kind": extras.get("sem_prazo_kind"),
        "bndes_classification": extras.get("bndes_classification"),
...
def _valid_prazo_iso(val: Any, record: Dict[str, Any]) -> Optional[str]:
    if not val:
        return None
    locale = infer_locale_from_source(str(record.get("fonte_recurso") or record.get("fonte") or "BNDES"))
...
def classify_apply_action(
    ...
    *,
    ...
    if not is_bndes_record(recrawl_item):
        return "reject", {"reason": "fonte_nao_bndes"}

    after_item = apply_deadline_backfill_to_payload(copy.deepcopy(recrawl_item), enabled=True)
    after_ex = _get_extras(after_item)
    preview = after_ex.get("_deadline_backfill_preview") or {}
    backfill = preview if preview.get("deadline_backfill_status") else extract_source_deadline_fields(recrawl_item)
    status = backfill.get("deadline_backfill_status") or "no_candidate"
    recrawl_ex = _get_extras(recrawl_item)
    ignored_dates = (
        after_ex.get("bndes_ignored_dates")
        or recrawl_ex.get("bndes_ignored_dates")
        or backfill.get("extras_patch", {}).get("bndes_ignored_dates")
        or []
    )
    deadline_kind = (
        after_ex.get("deadline_kind")
        or recrawl_ex.get("deadline_kind")
        or backfill.get("extras_patch", {}).get("deadline_kind")
    )
    ...
    meta = {
        ...
        "bndes_ignored_dates": ignored_dates,
    ...
    if after_prazo and deadline_kind != SUBMISSION_DEADLINE_KIND:
        return "reject", {
            **meta,
            "reason": "prazo_nao_e_submissao",
            "rejection_detail": f"deadline_kind={deadline_kind}",
        }

    has_db = bool(db_record)
    has_source_deadline = _has_bndes_deadline_in_payload(recrawl_item)

    if after_prazo and before_prazo and before_prazo == after_prazo:
        if deadline_kind == SUBMISSION_DEADLINE_KIND:
            return "unchanged", meta
        return "reject", {**meta, "reason": "prazo_nao_e_submissao"}

    if after_prazo and before_prazo and before_prazo != after_prazo:
        if deadline_kind == SUBMISSION_DEADLINE_KIND and status in (
            "new_deadline_found",
            "corrected_deadline_found",
        ):
            return "update", {**meta, "reason": "corrected_bndes_submission_deadline"}
        return "preserve", {**meta, "reason": "nao_sobrescrever_prazo_valido_diferente"}

    if after_prazo and (not before_prazo or not has_db):
        if has_source_deadline or status in (
            "new_deadline_found",
            "corrected_deadline_found",
            "replacement_candidate",
        ):
            return "update", {**meta, "reason": "novo_prazo_bndes"}
    ...
    if status == "preserved_existing" and before_prazo:
        if deadline_kind == SUBMISSION_DEADLINE_KIND:
            return "preserve", {**meta, "reason": "prazo_submissao_valido_existente"}
        return "reject", {**meta, "reason": "prazo_nao_e_submissao"}

    if status in ("new_deadline_found", "corrected_deadline_found", "replacement_candidate") and after_prazo:
        if deadline_kind == SUBMISSION_DEADLINE_KIND:
            reason = (
                "corrected_bndes_submission_deadline"
                if status == "corrected_deadline_found"
                else meta.get("reason") or "novo_prazo_bndes"
            )
            return "update", {**meta, "reason": reason}
...
def _pdf_detail_candidates(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for it in items:
        ex = _get_extras(it)
        sk = ex.get("sem_prazo_kind")
        if sk != "deadline_in_pdf_or_detail" and not (
            (ex.get("bndes_detail_links") or {}).get("pdf_urls") and not ex.get("pdf_texto_extraido")
        ):
            continue
        out.append(
            {
                ...
                "sem_prazo_kind": sk,
                "pdf_urls": (ex.get("bndes_detail_links") or {}).get("pdf_urls") or [],
                "detail_url": (ex.get("bndes_detail_links") or {}).get("detail_url"),
                "pdf_extract_error": (ex.get("bndes_pdf_extract") or {}).get("error"),
            }
        )
    return out


def run_bndes_deadline_dry_run(
    recrawl_items: List[Dict[str, Any]],
    *,
    db_by_link: Optional[Dict[str, Dict[str, Any]]] = None,
) -> Dict[str, Any]:
    recrawl_stats = stats_for_bndes_recrawl_items(recrawl_items)
    stats = {
    ...
    pdf_candidates = _pdf_detail_candidates(recrawl_items)
...
def validate_dry_run_report_recent(root: Path, max_age_hours: int = DRY_RUN_REPORT_MAX_AGE_HOURS) -> None:
    path = root / "outputs" / OUT_DIR_NAME / SUMMARY_FILENAME
    if not path.exists():
        raise RuntimeError(
            f"Relatório dry-run ausente: {path}. Execute dry_run_bndes_deadline_apply.py antes do apply."
        )
    age_h = (datetime.now(timezone.utc).timestamp() - path.stat().st_mtime) / 3600
    if age_h > max_age_hours:
        raise RuntimeError(f"Relatório dry-run expirado ({age_h:.1f}h > {max_age_hours}h). Reexecute o dry-run.")
...
def validate_candidates_batch(
    ...
    *,
    ...
    for i, cand in enumerate(candidates):
        if not is_bndes_record(cand.get("payload_after") or cand):
            raise RuntimeError(f"Candidato {i} não é BNDES — apply abortado.")
...
def apply_bndes_candidates(
    ...
    *,
    ...
    for cand in candidates:
        ...
        if not is_bndes_record(payload):
        ...
        if existing and not is_bndes_record(existing):
            raise RuntimeError(
